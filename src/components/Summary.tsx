...
export function ResultBar({ result }: { result: EarResult }) {
  ...
  return (
    <div
      className="sticky bottom-0 z-30 border-t border-border bg-surface-raised"
    ...
    >
      ...
      >
        <div
          className={cn('flex items-center gap-2 text-sm font-medium', ready ? 'text-success' : 'text-danger')}
          role="status"
          aria-live="polite"
        >
          {ready ? (
            ...
            <li key={v.id} className="flex items-start gap-2.5">
              <Icon
                className={cn(
                  ...
                  v.notApplicable ? 'text-fg-subtle' : v.ok ? 'text-success' : 'text-danger',
                )}
                aria-hidden
              />
              <div className="min-w-0">
                <span className="text-xs font-semibold text-fg-muted">{v.id}</span>
                <p
                  className={cn(
                    'text-sm',
                    v.notApplicable ? 'text-fg-subtle' : v.ok ? 'text-fg-muted' : 'text-danger',
                  )}
                >
                  {v.message}
                </p>
        ...
        <div className="mt-4 space-y-2 border-t rule-hair pt-4">
          {result.warnings.map((w) => (
            <div key={w.code} className="flex items-start gap-2.5">
              ...
              <p className="text-sm text-fg-muted">{w.message}</p>
            </div>
          ))}
        </div>
      )}
